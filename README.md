...
SnapFrame is not published in the official Home Assistant add-on store (it needs `full_access`/`SYS_ADMIN` to mount CIFS, which isn't allowed for store add-ons — see [Security notes](#security-notes)). Installing it as a **local add-on** takes about five minutes.

### Step 1 — Get the files onto your Home Assistant instance

The add-on needs to live at `/addons/snapframe/` on the machine running Home Assistant. Pick whichever of these is easiest for you:
...
3. Copy the entire contents of this repository into it

**Option B – SSH / Terminal add-on (recommended if you're comfortable with a shell)**
```bash
cd /addons
git clone https://github.com/emo546/ha-snapframe snapframe
```

**Option C – Studio Code Server add-on**
1. Install the "Studio Code Server" add-on from the store and open it
2. Open a terminal inside the code editor and run the same `git clone` command as above
