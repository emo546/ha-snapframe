...
- **Supervisor watchdog** – `watchdog` points at the new unauthenticated `GET /health`, so a frozen add-on gets restarted instead of quietly serving nothing. The probe is deliberately exempt from Basic Auth: the Supervisor has no credentials, and a 401 would restart-loop the add-on.
