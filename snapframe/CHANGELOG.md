...
- **A daily forecast was interpolated as if it were hourly.** `weather.get_forecasts` with `type: daily` — which the documentation offers for integrations without an hourly forecast — returns one entry per day carrying that day's *maximum*. 3.1.0 interpolated between those maxima, so a frame showed roughly the afternoon high all morning (~25 °C at 6 a.m. against a real 14 °C). Points more than 3 hours apart are no longer treated as neighbouring hours — and the same check now also applies right at the edges of the forecast (before its first point or after its last), where it had been skipped because there was no second point to compare against; a single coarse point sitting close to "now" no longer slips through and displays its day's maximum. With a coarse forecast the frame keeps showing the last measurement instead, with its age printed underneath as before.
